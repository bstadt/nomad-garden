...
<LatexFormula formula={'$ a_{ij} = l(V_i, V_j)$'}/>

The link function is interpreted as the way in which we measure the strength of the connection between two verticies in our graph.
...
This link function leads to a very concise definition of the adjacency matrix.
Let X be the matrix of vectors associated with a vertex set.
We have:
<LatexFormula formula={'$ X \\in \\mathbb R^{(|V|, d)}$'}/>
<LatexFormula formula={'$ A = XX^T$'}/>
...
<LatexFormula formula={'$Q = q_\\theta(X)$'}/>
<LatexFormula formula={'$K = k_\\phi(X)$'}/>
<LatexFormula formula={'$V = v_\\psi(X)$'}/>
<LatexFormula formula={'$S = \\text{softmax}\\big(QK^T/\\sqrt d \\; \\big)V$'}/>
where q, k, and v denote feed forward neural networks with learnable parameters.

The interaction between the Q and K looks *very* similar to the linear link function we saw earlier.
...
It draws *connections* between several *heterogeneous* domains, and it will undoubtedly trigger a *multiplicity* of reactions that will *rupture* and connections that I tried to make.
