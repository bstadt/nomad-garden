...
This recipe could be applied to <em>any</em> problem, given access to sufficient data about that problem, and sufficiently many GPUs.
There was now a physically realizable way to utilize the thread-memory-time tradeoff, and it was only limited by the hardware on the GPUs themselves:

> All of our experiments suggest that our results can be improved simply by waiting for faster GPUs and bigger datasets to become available.
- Krizhevsky et al. (2012)

Though Krizhevsky was correct that faster GPUs could improve his results his that observation misses the forest for the trees (which is easy for me to say in the fullness of retrospect).
...
<div>
    <iframe
        width="600"
        height="480"
        src="https://www.youtube.com/embed/ooBt_di8DLs?si=Yi-c6QUYckYVSayM&amp;start=393"
...
Moreover Einstein's spacetime comes equipped with an intrinsic tradeoff between space and time: the [spacetime-interval](https://en.wikipedia.org/wiki/Spacetime#Spacetime_interval).
Concretely, the spacetime interval between two events is expressed:
<LatexFormula formula={'$ (\\Delta S)^2 = (\\Delta ct)^2 - (\\Delta x)^2 - (\\Delta y)^2 - (\\Delta z)^2 $'}/>
Where t is time, x, y, and z are physical coordinates, and c is the speed of light.
The critical thing to notice about this formula is that for a fixed spacetime-interval, we can decrease the time component of the interval by increasing the space components of the interval.
...
To fully appreciate the relationship between computation and spacetime-intervals, we need a slightly more precise definition of physical computation.
...
In short, we can think of a physical computation P as a function that transforms one physical state into another.
From Einstein, we know that the physical start and end states of the computation are points in spacetime, and thus, span a spacetime interval.
In this sense any computation can be said to have a physical spacetime cost, which is exactly the measure of the interval required to bridge its start and end states.
...
<div>
    <iframe
        width="600"
        height="480"
        src="https://www.youtube.com/embed/yb0y6MRwmd4?si=p4gInzoAmJasf2PJ&amp;start=226"
        allow="accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
        allowFullScreen
        title="Embedded youtube"
    />
